...
void stageSceneHook()
{
    ...
    if (al::isPadHoldR(-1) && al::isPadTriggerL(-1))
    {
        isFlying = !isFlying;
    }

    al::calcFrontDir(&fwddir, p1);

    p1pos = al::getTrans(p1);

    if (isFlying)
    {
        al::setVelocityZero(p1);
        if (al::getLeftStick(-1)->x >= 0.01) 
        {
            al::addVelocityX(p1, fwddir.x * al::getLeftStick(-1)->x * flySpeed);
            al::addVelocityZ(p1, fwddir.z * al::getLeftStick(-1)->x * flySpeed);
        }
        if (al::getLeftStick(-1)->x <= 0.01) 
        {
            al::addVelocityX(p1, fwddir.x * (al::getLeftStick(-1)->x * -1) * flySpeed);
            al::addVelocityZ(p1, fwddir.z * (al::getLeftStick(-1)->x * -1) * flySpeed);
        }
        if (al::getLeftStick(-1)->y >= 0.01) 
        {
            al::addVelocityX(p1, fwddir.x * al::getLeftStick(-1)->y * flySpeed);
            al::addVelocityZ(p1, fwddir.z * al::getLeftStick(-1)->y * flySpeed);
        }
        if (al::getLeftStick(-1)->y <= 0.01) 
        {
            al::addVelocityX(p1, fwddir.x * (al::getLeftStick(-1)->y * -1) * flySpeed);
            al::addVelocityZ(p1, fwddir.z * (al::getLeftStick(-1)->y * -1) * flySpeed);  
        }
        if (al::isPadHoldB(-1)) { al::setTransY(p1, al::getTrans(p1).y + flySpeed); }
        if (al::isPadHoldA(-1)) { al::setTransY(p1, al::getTrans(p1).y - flySpeed); }
    }
